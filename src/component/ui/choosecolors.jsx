...
const SelectColor = ({ color, label, onClick }) => {
  return (
    <button
      onClick={onClick}
      className="group flex flex-col items-center space-y-2 transition-all duration-300 hover:scale-105 focus:outline-none focus:ring-2 focus:ring-blue-300 rounded-xl"
      aria-label={`Select ${label} color`}
    >
      <Card
        className={`w-[110px] h-[100px] sm:w-[140px] sm:h-[140px] md:w-[90px] md:h-[90px] lg:w-[90px] lg:h-[90px] rounded-xl overflow-hidden ${Colors.borderGray} bg-white shadow-md hover:shadow-xl transition-all duration-300 group-hover:ring-2 group-hover:ring-blue-300`}
      >
